...
VERSION_SOURCE=splunk-tracer-jre/src/main/java/com/splunk/tracer/jre/Version.java
perl -pi -e 's/(\d+)\.(\d+)\.(\d+)/ $ENV{NEW_VERSION} /ge' ${VERSION_SOURCE}

# Commit the changes
git add benchmark/pom.xml
git add examples/pom.xml
git add splunk-tracer-jre/pom.xml
git add splunk-tracer-jre-bundle/pom.xml
git add pom.xml
git add shadow/pom.xml
git add splunk-tracer-jre/src/main/java/com/splunk/tracer/jre/Version.java
